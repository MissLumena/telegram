...
Этот проект — Telegram-бот. Тесты, которые проверяют поведение самого бота и бизнес-логику, выполняются так:
...
Сценарии Playwright (`npm run test:e2e`) актуальны только в случае наличия веб-интерфейса. В текущем репозитории Telegram клиент работает через `telegraf`, поэтому браузерные E2E тесты не подходят без дополнительного фронтенда.
