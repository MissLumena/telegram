...
serve(async (req) => {
  try {
    ...
    const { action } = body;
    console.log("sheets-appointments: action", action);
    ...
    if (action === "list_available_slots") {
      const { service_id, master_id, date } = body;
      if (!date) {
      ...
      console.log("sheets-appointments: availableSlots", availableSlots);
      return Response.json({ availableSlots });
    }

    if (action === "create") {
      const appointment = body.appointment;
      if (!appointment || !appointment.date || !appointment.time || !appointment.service_id) {
      ...
      if (appointment.master_id === "any") {
        const eligibleMasters = mastersForService(appointment.service_id);
        const bookedIds = new Set(
          active
            .filter((row) => row.get("date") === appointment.date && row.get("time") === appointment.time)
        ...
        appointment.master_id = freeMaster.id;
        appointment.master_name = freeMaster.name;
      } else {
        const slotTaken = active.some(
          (row) => row.get("date") === appointment.date && row.get("time") === appointment.time && row.get("master_id") === appointment.master_id
        );
        if (slotTaken) {
          return Response.json({ error: "slot unavailable" }, { status: 409 });
        }
      ...
      await sheet.addRow({ ...appointment, status: "active" });
    ...
    console.error("sheets-appointments: unknown action", action);
